...
export function MapPage() {
  ...
  if (!hasValidJob) {
    return (
      <Stack gap={3}>
        <Heading size="lg">Knowledge map</Heading>
        <Text>
          Pick a job from the{" "}
  ...
  return (
    <Stack gap={6}>
      <Heading size="lg">Knowledge map · Job #{jobId}</Heading>
    ...
    <Stack gap={6}>
      <Box display="flex" alignItems="center" justifyContent="space-between" flexWrap="wrap" gap={2}>
        ...
        <Link asChild color="blue.300" fontSize="sm">
          <RouterLink to={`/gaps?mapping_job_id=${summary.job_id}`}>Detect gaps on this map →</RouterLink>
        </Link>
      </Box>
